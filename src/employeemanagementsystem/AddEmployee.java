...
public class AddEmployee extends JFrame implements ActionListener {
    ...
    JTextField tname,tfname,tadd,tsal,taadhar,tphone,tdesg,temail;
    JComboBox tedu;
    JButton back,add;
    JDateChooser dcdob;
    JLabel leid;
    ...
    public AddEmployee(){
        ...
        String course[]={"BBA","BCA","BA","B.COM","B.TECH","BSC","MBA","MB","MCA","MA","M.TECH","MSC","PHD"};
        tedu=new JComboBox(course);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == add){
            ...
            String empid =leid.getText();
           
           try{
               Conn conn=new Conn();

                String query;  
                query = "insert into employee values('"+name+"','"+fname+"','"+dob+"','"+salary+"','"+address+"','"+phone+"','"+email+"','"+education+"','"+designation+"','"+aadhar+"','"+empid+"')";
                conn.s.executeUpdate(query);    
               JOptionPane.showMessageDialog(null,"Details add Successsfully");
               setVisible(false);
               new Home();
